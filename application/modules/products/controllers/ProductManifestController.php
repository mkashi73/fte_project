...
class ProductManifestController extends MX_Controller 
{
	...
	public function GenerateLoadsheetExcel()
	{
		$tokenData = getTokenData('token');
			...
			) 
		{
			...
			$spreadsheet->getActiveSheet()->setCellValue('A1', "AWB: ")->getStyle('A1')->applyFromArray($styleArray);			
			$spreadsheet->getActiveSheet()->setCellValue('C1', "Flight Date: ")->getStyle('B1')->applyFromArray($styleArray);			
			$spreadsheet->getActiveSheet()->setCellValue('E1', "RUN #: ")->getStyle('C1')->applyFromArray($styleArray);
			


			$spreadsheet->getActiveSheet()->setCellValue('A3', "Sr No ")->getStyle('A3')->applyFromArray($styleArray);
			$spreadsheet->getActiveSheet()->setCellValue('B3', "CN DATE")->getStyle('B3')->applyFromArray($styleArray);
			$spreadsheet->getActiveSheet()->setCellValue('C3', "PARTY NAME")->getStyle('C3')->applyFromArray($styleArray);
			$spreadsheet->getActiveSheet()->setCellValue('D3', "CN #")->getStyle('D3')->applyFromArray($styleArray);
			$spreadsheet->getActiveSheet()->setCellValue('E3', "DESTINATION")->getStyle('E3')->applyFromArray($styleArray);
			$spreadsheet->getActiveSheet()->setCellValue('F3', "STATION")->getStyle('F3')->applyFromArray($styleArray);
			$spreadsheet->getActiveSheet()->setCellValue('G3', "PCS")->getStyle('G3')->applyFromArray($styleArray);
			$spreadsheet->getActiveSheet()->setCellValue('H3', "WEIGHT")->getStyle('H3')->applyFromArray($styleArray);
			$spreadsheet->getActiveSheet()->setCellValue('I3', "CN AMOUNT")->getStyle('I3')->applyFromArray($styleArray);
			$spreadsheet->getActiveSheet()->setCellValue('J3', "RECEIVED")->getStyle('J3')->applyFromArray($styleArray);
			$spreadsheet->getActiveSheet()->setCellValue('K3', "BALANCE AMOUNT")->getStyle('K3')->applyFromArray($styleArray);
			...
			foreach( $data['result'] AS $data ) {
				$total_amount = $data['CONSIGNEE_AMOUNT'] + $data['BALANCE_AMOUNT'];
				$spreadsheet->getActiveSheet()->setCellValue("A{$i}", "{$serialNo}")->getStyle("A{$i}")->applyFromArray($detaiStyleArray);
				$spreadsheet->getActiveSheet()->setCellValue("B{$i}", "{$data['E_DATE_TIME']}")->getStyle("B{$i}")->applyFromArray($detaiStyleArray);
				$spreadsheet->getActiveSheet()->setCellValue("C{$i}", "{$data['SHIPPER_NAME']}")->getStyle("C{$i}")->applyFromArray($detaiStyleArray);
				$spreadsheet->getActiveSheet()->setCellValue("D{$i}", "{$data['CN_NUMBER']}")->getStyle("D{$i}")->applyFromArray($detaiStyleArray);
				$spreadsheet->getActiveSheet()->setCellValue("E{$i}", "{$data['CONSIGNEE_COUNTRY']}")->getStyle("E{$i}")->applyFromArray($detaiStyleArray);
				$spreadsheet->getActiveSheet()->setCellValue("F{$i}", "{$data['STATION_NAME']}")->getStyle("F{$i}")->applyFromArray($detaiStyleArray);
				$spreadsheet->getActiveSheet()->setCellValue("G{$i}", "{$data['QUANTITY']}")->getStyle("G{$i}")->applyFromArray($detaiStyleArray);
				$spreadsheet->getActiveSheet()->setCellValue("H{$i}", "{$data['PRODUCT_NET_WEIGHT']}")->getStyle("H{$i}")->applyFromArray($detaiStyleArray);
				$spreadsheet->getActiveSheet()->setCellValue("I{$i}", "{$total_amount}")->getStyle("I{$i}")->applyFromArray($detaiStyleArray);
				$spreadsheet->getActiveSheet()->setCellValue("J{$i}", "{$data['CONSIGNEE_AMOUNT']}")->getStyle("J{$i}")->applyFromArray($detaiStyleArray);
				$spreadsheet->getActiveSheet()->setCellValue("K{$i}", "{$data['BALANCE_AMOUNT']}")->getStyle("K{$i}")->applyFromArray($detaiStyleArray);
				// debug($data);
				$serialNo++;
				$i++;
			}
